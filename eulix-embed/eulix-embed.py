...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ep.add_argument("-m", "--model",         default="sentence-transformers/all-MiniLM-L6-v2",
                    metavar="NAME",          help="HuggingFace model name")
    ep.add_argument("-d", "--device",        default=None,                  metavar="DEVICE",
                    help="cuda / mps / cpu             [default: auto]")
    ep.add_argument("-b", "--batch-size",    type=int, default=None,        metavar="N",
                    help="Batch size                   [default: auto]")
    ep.add_argument("--max-chunk",           type=int, default=2000,        metavar="N",
                    help="Max chunk chars              [default: 2000]")
    ep.add_argument("--quantize",            action="store_true",
    ...
    ep.add_argument("--debug",               action="store_true",
                    help="Print debug info during pipeline")
    qp = sub.add_parser("query", help="Generate embedding for a query string (one-shot)")
    qp.add_argument("-q", "--query",  default="",                                   metavar="TEXT",
                    help="Query text to embed")
    qp.add_argument("-m", "--model",  default="sentence-transformers/all-MiniLM-L6-v2",
                    metavar="NAME",   help="HuggingFace model name")
    qp.add_argument("-f", "--format", default="json",                                metavar="FMT",
                    help="json | binary  [default: json]")
    sp = sub.add_parser("serve", help="Long-lived stdin/stdout embedding server (avoids model reload)")
    sp.add_argument("-m", "--model",      default="sentence-transformers/all-MiniLM-L6-v2",
                    metavar="NAME",       help="HuggingFace model name  [default: all-MiniLM-L6-v2]")
    sp.add_argument("-d", "--device",     default=None, metavar="DEVICE",
                    help="cuda / mps / cpu  [default: auto]")
    sp.add_argument("-b", "--batch-size", type=int, default=None, metavar="N",
