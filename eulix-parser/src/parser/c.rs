...
impl CParser {
    ...
    fn pre_scan_macros(src: &str) -> HashMap<String, String> {
        ...
        let re = Regex::new(r"(?m)^#define\s+([A-Za-z_]\w*)(?:\([^)]*\))?\s+(.+)$").unwrap();
    ...
    pub fn parse(&self) -> Result<FileData, String> {
        ...
        let fn_ptr_map = self.build_fn_ptr_map(&root);
        ...
        Ok(FileData {
            ...
            functions: self.extract_functions(&root, &fn_ptr_map),
    ...
    fn detect_security_patterns(&self) -> Vec<SecurityNote> {
        ...
        let patterns = vec![
            ...
            (
                r"scanf|fscanf",
                "unsafe_input",
                "Potentially unsafe input function",
            ),
            (
    ...
    //  Auto-tagging / importance (unchanged logic, kept for completeness)
    ...
    fn auto_tag_function(
        ...
        if calls.iter().any(|c| {
            c.callee.contains("syscall") || c.callee.contains("ioctl") || c.callee.contains("fcntl")
        }) {
            tags.push("system-call".to_string());
        }
        if calls.iter().any(|c| {
            c.callee.contains("pthread") || c.callee.contains("fork") || c.callee.contains("thread")
        }) {
            tags.push("concurrent".to_string());
            tags.push("threading".to_string());
        }
        if calls.iter().any(|c| {
            c.callee.contains("strcpy")
